@php
    /**
     * Image entry that keeps a large upload inside its card and opens a
     * zoomable lightbox on click: scroll/pinch to zoom, drag or swipe to pan.
     *
     * Styling is inline on purpose — this panel uses Filament's precompiled
     * CSS, which only ships the utility classes Filament itself uses, so custom
     * Tailwind classes here would silently render as unstyled markup.
     *
     * Expects `$url` (nullable) and optional `$caption`.
     */
    $url ??= null;
...
@else
    ...
    >
        {{-- Card: the image is contained, so an oversized upload can never
             push past the infolist card. --}}
        <div style="max-width:28rem;overflow:hidden;border:1px solid rgba(128,128,128,.25);border-radius:.75rem;background:rgba(128,128,128,.06);padding:.5rem">
            <button type="button" x-on:click="show()" style="display:block;width:100%;cursor:zoom-in;border:0;background:none;padding:0">
                <img
                    src="{{ $url }}"
                    alt="{{ $caption }}"
                    style="display:block;margin:0 auto;max-height:20rem;width:100%;object-fit:contain;border-radius:.5rem"
                    loading="lazy"
                >
            </button>
